...
- STARTER FERTILIZER (high phosphorus: 12-24-12, 18-24-12, 24-25-4 Starter): most effective AT THE TIME OF SEEDING (day 0), but CAN still provide benefit during active germination and early seedling establishment — up to 4 weeks post-seeding per PSU Extension guidance. If notes indicate seeding occurred WITHIN 4 WEEKS and seeds are actively germinating or seedlings are emerging, a light starter fertilizer application can still be beneficial. Frame it as: "If you haven't applied starter fertilizer yet, it can still help during early germination — apply a light rate and water in gently." After 4 weeks or once seedlings are 2+ inches tall, the starter window has closed — focus on establishment mowing and wait for the regular program.
- REGULAR FERTILIZER PROGRAM: Do NOT begin a regular maintenance fertilizer program until full establishment (after 2–3 mowings at full height, typically 6–8 weeks post-seeding). At this stage, begin with a light application (0.5 lbs slow-release N/1000 sqft).
...
- TALL FESCUE SUMMER DORMANCY: If the homeowner is unable or unwilling to irrigate through the summer, note that tall fescue can be allowed to go dormant in summer heat as an acceptable management option — reduce irrigation to 0.5 inches every 2–3 weeks to keep crown alive (survival moisture only, not growth irrigation), then resume full irrigation in fall when highs consistently fall below 85–90°F. Do NOT set the recovery threshold at 75°F — that is too conservative; most extension sources recommend resuming fall irrigation when temperatures fall into the 80–85°F range. Dormancy is an extension-endorsed survival strategy, not a failure. Mention this option when tall fescue is in drought stress.`
  ...
  if (isDryConditions && !isDroughtStress) {
    warnings.push(
      `⚠️ DRY CONDITIONS FUNGICIDE CONSTRAINT (MANDATORY): Humidity is ${humidity}% with no recent rainfall — these conditions do NOT support fungal disease development or spread. ABSOLUTE HARD RULE: Do NOT recommend fungicide application of any kind. This means: NO summer patch drench, NO necrotic ring spot treatment, NO dollar spot spray, NO gray leaf spot treatment, NO mention of azoxystrobin, propiconazole, myclobutanil, Headway, Heritage, Armada, or ANY other fungicide product. Disease topics may ONLY be mentioned as: "If conditions become more humid in the future, watch for X" — never as a current or imminent treatment recommendation.`
    );
  }
