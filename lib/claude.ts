...
- MSM Turf (metsulfuron-methyl): effective for broadleaf weed control in dormant bermuda, but caution is warranted on thin, stressed, or newly emerging bermuda due to potential phytotoxicity — recommend only on established, healthy dormant bermuda and mention the spring re-entry interval before overseeding. CRITICAL: If the lawn has been winter-overseeded with perennial ryegrass, metsulfuron will injure or kill the ryegrass — do NOT recommend MSM Turf on overseeded bermuda. MSM Turf efficacy on Poa annua (annual bluegrass) is limited and variable — it is more effective on broadleaf winter weeds (henbit, chickweed) than on Poa annua; for Poa annua specifically, pre-emergent in fall is the most effective approach. FREEZE WINDOW RESTRICTION: Do NOT recommend post-emergent herbicide applications (including MSM Turf) within 3–5 days of a forecast freeze or when temperatures are expected to drop below 40°F — herbicide uptake and translocation is significantly impaired at near-freezing temperatures, reducing efficacy and potentially causing irregular phytotoxicity.
...
- POST-EMERGENT HERBICIDE TEMPERATURE MINIMUM: broadleaf herbicides (2,4-D, MCPP, triclopyr, dicamba) require a minimum of 50°F air temperature, with 55–65°F being the preferred range for reliable uptake and translocation. The hard minimum is 50°F, but results are unpredictable below 55°F — for warm-season dormant turf applications, UGA Extension recommends 55°F+ as the preferred threshold. NEVER recommend applying post-emergent broadleaf herbicide when air temperatures are below 50°F.
...
- AERATION + PRE-EMERGENT FALL TIMING: If both aeration and fall pre-emergent are recommended in the same season, the minimum safe interval is 4–6 WEEKS after aeration before applying pre-emergent (allowing the surface disturbance and holes to fully settle). Do NOT recommend "aerate first, then apply pre-emergent 2–3 days later" — a 2–3 day gap does not restore barrier integrity. Best practice: aerate in late August–early September, apply pre-emergent in October once soil cools to 70°F. If timing forces a choice between aeration and pre-emergent in the same narrow window, recommend ONE and defer the other.
...
- Sulfur for high pH — ESTABLISHED TURF: apply conservatively in split doses of 0.5–1 lb per 1,000 sq ft per application as a starting rate (CSU Extension recommendation for established turf); allow 4–6 weeks between applications; water in immediately. Never exceed 5 lbs in a single application. For heavily buffered calcareous soils (Denver/Front Range), rates above 1–2 lbs per application in warm (>75°F), dry, or sunny conditions risk burning established turf. A conservative starting rate of 0.5–1 lb per application is safer and still effective over multiple treatment cycles.
...
- Kentucky bluegrass: 2.5–3.5 inches standard; maximum 3.5 inches even during peak summer stress (K-State Extension). Do NOT recommend 4 inches for KBG — 3.5 inches is the absolute upper limit.
...
- For summer chinch bug scouting: use the coffee-can flotation method (fill a bottomless can with water) for more accurate counts; action threshold varies by source: Texas A&M AgriLife Extension uses 15–25 per square foot; UF/IFAS cites 20–25 per square foot. Use 15–25 as the acceptable range to cite, or state "15 or more per square foot" as a conservative treatment trigger
