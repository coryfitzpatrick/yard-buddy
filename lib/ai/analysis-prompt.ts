...
- DO NOT EXPAND A SINGLE VISIBLE ISSUE INTO MULTIPLE SPECULATIVE SUB-ISSUES. If you see drought stress, list ONLY drought_stress, NOT also "possible dormancy onset", "risk of stand loss", "potential disease pressure". If you see fungal mycelium / damping-off, list ONLY ONE of damping_off, pythium_blight, or fungus — pick the most accurate single label rather than listing both the general category and a pathogen-specific variant. HOWEVER: a homeowner-confirmed or visible CAUSE (overwatering, overfertilization, scalping, compaction, drought_stress) is a SEPARATE issue from a visible DISEASE — list both when both apply. Example: photo shows damping_off + homeowner notes 4x daily watering → list ["damping_off", "overwatering"], not just ["damping_off"]. Inflated issue sets get penalized, but undercounting causes is also penalized.
...
- DO NOT TREAT HOMEOWNER-NOTES-DESCRIBED PROBLEMS AS PHOTO-CONFIRMED. The issues array reflects what is VISIBLE in the photos, not what the homeowner described in their notes. If the homeowner writes "I have bare spots in both yards" but the submitted photos show only healthy canopy, do NOT add "bare_spots" to the issues array — instead, address it CONDITIONALLY in recommendations ("If you're seeing bare spots elsewhere on the property, here's how to handle them..."). This keeps the issues array faithful to photographic evidence and avoids over-diagnosing based on a homeowner's self-report.
