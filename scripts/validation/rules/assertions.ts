...
const mowingHeightInRange: Rule = {
  ...
  check(scenario, response): RuleResult {
    ...
    const nonMowingCtx = /\b(?:water(?:ing|ed|s)?|rain(?:fall|ing)?|irrigat\w*|deep|penetrat\w*|per\s+week|per\s+day|days?|week|month|topsoil|mulch|thatch|reach(?:es|ed|ing)?|tall(?:er)?|grow(?:ing|s|th|n)?|overgrown|accumulate[ds]?|remove[ds]?|more\s+than|one.?third|at\s+once|at\s+a\s+time|one.?third\s+rule|bermuda(?:grass)?|rye(?:\s*grass)?|centipede|zoysia|buffalo|bluegrass|fescue|unlike|compar(?:ed|ing)?|rather\s+than|instead|contrast|other\s+grass|different\s+grass|warm.season|cool.season|below|never\s+below|minimum|minimum\s+height|scalp(?:ing)?|floor|lower\s+limit|raise|raising|raised|higher|increase|lift|golf|fairway|reel\s+mow|sport(?:s)?\s+turf|professional)\b/i;
