 ...
 *
 * Set DEMO_EMAIL env var to target a specific account (default: yardanalyzer+demo@gmail.com)
...
const DEMO_EMAIL = process.env.DEMO_EMAIL ?? "yardanalyzer+demo@gmail.com";
...
async function main() {
  ...
  const frontAnalysis2 = await db.lawnAnalysis.create({
    data: {
      yardSectionId: frontYard.id,
      imageUrls: [`${BASE_URL}/fair-lawn.jpg`],
      healthScore: 74,
  ...
  const frontAnalysis3 = await db.lawnAnalysis.create({
    data: {
      yardSectionId: frontYard.id,
      imageUrls: [`${BASE_URL}/lush-green.jpg`],
      healthScore: 88,
