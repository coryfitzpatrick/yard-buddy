...
describe("ScheduleRecommendationCard - watering", () => {
  it("state A: shows empty state when no analysis", () => {
    render(<ScheduleRecommendationCard kind="watering" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={null} effective={baseEffective} />);
    expect(screen.getByText(/run an analysis/i)).toBeInTheDocument();
  });

  it("state B: shows neutral confirmation when not deviating", () => {
    render(<ScheduleRecommendationCard kind="watering" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={baseAnalysis} effective={baseEffective} />);
  ...
  it("state C: shows Apply and Ignore when deviating and not dismissed", () => {
    ...
    render(<ScheduleRecommendationCard kind="watering" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={dev} effective={baseEffective} />);
  ...
  it("collapses state D to B when effective schedule matches saved suggestion", () => {
    ...
    render(<ScheduleRecommendationCard kind="watering" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={dev} effective={matching} />);
  ...
  it("state B: shows manual setup CTA when no schedule and no AI suggestion", () => {
    ...
    render(<ScheduleRecommendationCard kind="watering" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={noSuggestion} effective={empty} />);
...
describe("ScheduleRecommendationCard - mowing", () => {
  ...
  it("renders mowing-specific state C with height in inches", () => {
    ...
    render(<ScheduleRecommendationCard kind="mowing" sectionId="sec_1" yardSlug="yard-1" latestAnalysis={mowingAnalysis} effective={effective} />);
    expect(screen.getByRole("button", { name: /apply/i })).toBeInTheDocument();
